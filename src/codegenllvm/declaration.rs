...
use crate::ast::{Extern, Function, Global, Module, Prototype};
...
impl<'ctx> Compiler<'ctx> {
    ...
    fn compile_proto(&mut self, module: &Module, proto: &Prototype) -> CompileResult<FunctionValue<'ctx>> {
        let ret_type = Compiler::inkwell_type(&self.context, &proto.return_type);
    ...
    pub fn compile_func(&mut self, module: &Module, func: &Function) -> CompileResult<FunctionValue<'ctx>> {
        ...
        for (i, arg) in fn_val.get_param_iter().enumerate() {
            ...
            self.bindings.insert(proto.args[i].0.clone(), TypedValue::new(alloca.into(), arg_ty));
        }
