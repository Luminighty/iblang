...
macro_rules! assert_next_extern {
    ($parser: expr, $name: expr) => {
        match $parser.next() {
            Ok(Declaration::Extern(e)) => {
...
fn test_declaration() {
    ...
    assert_next_global!(parser, "abc", AstExprKind::Literal(Literal::Number(123)), true);

    let mut parser = create_ast("const abc = 'a';");
    assert_next_global!(parser, "abc", AstExprKind::Literal(Literal::Char('a')), false);
    ...
    let f = assert_next_extern!(parser, "putchar");
    assert_eq!(f.prototype.args.len(), 0);
    assert_eq!(f.prototype.return_type, AstFlowType::Void);
}
...
#[test]
fn test_prototype() {
    let mut parser = create_ast(
        "extern foo(bar: int, baz: char, other: float, is_good: bool): int"
    );
    let e = assert_next_extern!(parser, "foo");
    assert_eq!(e.prototype.args.len(), 4);
    assert_eq!(e.prototype.args[0], ("bar".to_string(), AstTypeIdent::Atomic(Atomic::int())));
    assert_eq!(e.prototype.args[1], ("baz".to_string(), AstTypeIdent::Atomic(Atomic::char())));
    assert_eq!(e.prototype.args[2], ("other".to_string(), AstTypeIdent::Atomic(Atomic::Float)));
    assert_eq!(e.prototype.args[3], ("is_good".to_string(), AstTypeIdent::Atomic(Atomic::bool())));
    assert_eq!(e.prototype.return_type, AstFlowType::Some(AstTypeIdent::Atomic(Atomic::int())));
}
